...
get_aandacht <- function(q) {
  amcat4r::query_documents("tk2023_radio_tv", queries = q,
                           fields = c('_id', 'publisher','date','text', 'speakernum','start','end','won', 'embedding'),
...
aandacht <- map(queries, get_aandacht, .progress=T) |>
  list_rbind()
...
googlesheets4::gs4_deauth()
namen <- read_sheet('https://docs.google.com/spreadsheets/d/10qSZponLZ06Rv5KOq83pn7vUpV4Fh5LdVlxhF9t6rgE/edit#gid=0', sheet = 1)
namen=namen|>
  left_join(wons) |>
  select(.id, spreker=speaker_name)

amcat4r::update_documents("tk2023_radio_tv", documents = namen)
