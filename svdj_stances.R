...
meta = meta|>
  filter(! grepl('België 1|BVN|NPO 1|NPO 2|NPO 3|INHOUD|Kruiswoordtest', title))
...
units = create_units(d3, id = 'sent_id', set_text('sent_text', sent_text, bold=T, before = before, after =after )) 
class(units)

d2=sents
d2=d2|>
  filter(! sent_id %in% n$unit_id)
...
write_csv(artcodings,"data/tk2023_issue1_nl.csv")
